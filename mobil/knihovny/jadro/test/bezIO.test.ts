...
describe('jádro je čistá knihovna', () => {
  ...
  it('nemá běhové závislosti — importuje jen vlastní soubory', () => {
    for (const { jmeno, obsah } of zdrojaky()) {
      const importy = [...obsah.matchAll(/from\s+['"]([^'"]+)['"]/g)].map((m) => m[1]);
      expect(importy.filter((cesta) => !cesta?.startsWith('./')), jmeno).toEqual([]);
    }
  });
});
